...
"
let s:table_block_map_list_filepaths_semantic_early = []
let s:table_block_map_list_filepaths_semantic_later = []

if exists('b:did_nftables_table_block_map')
  call nftables#syntax#log('INFO', 'Skipped table_block_map (already loaded for buffer: ' . bufname('%') . ')')
  finish
endif
...
try
  ...
  for s:this_semantic_file in s:table_block_map_list_filepaths_semantic_early
    call nftables#syntax#log('OK', 'Loading ' . s:this_semantic_file)
    call nftables#syntax#load(s:this_semantic_file)
    call nftables#syntax#log('OK', 'Loaded ' . s:this_semantic_file)
  endfor
  call nftables#syntax#debug('Loading table_block_map syntax ...' )
...
" ************* END table_block 'map' map_block ***************
  ...
  for s:this_semantic_file in s:table_block_map_list_filepaths_semantic_later
    call nftables#syntax#log('OK', 'Loading ' . s:this_semantic_file)
    call nftables#syntax#load(s:this_semantic_file)
    call nftables#syntax#log('OK', 'Loaded ' . s:this_semantic_file)
  endfor
  call nftables#syntax#log('INFO', 'Loaded table_block_map for buffer: ' . bufname('%'))
catch
  call nftables#syntax#log('ERROR', 'Failed to define table_block_map.vim: ' . v:exception . ' at line ' . line('.') . ' in ' . expand('<sfile>:t') . ' at ' . v:throwpoint)
endtry
...
let b:nft_did_nftables_table_block_map = v:true
